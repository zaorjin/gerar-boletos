...
var Beneficiario = (function () {
	...
	Beneficiario.prototype.comNossoNumero = function (_nossoNumero, banco) {
		if (banco instanceof Sicoob) _nossoNumero = pad(_nossoNumero, 7, '0');
  		else if (banco instanceof Santander) _nossoNumero = pad(_nossoNumero, 12, '0');
  		else if (banco instanceof Sicredi) _nossoNumero = pad(_nossoNumero, 5, '0');
  		else if (!(banco instanceof BancoBrasil) || this.getNumeroConvenio().length < 7)
  			_nossoNumero = pad(_nossoNumero, 11, '0');
 		else _nossoNumero = this.getNumeroConvenio().substring(0, 7) + pad(_nossoNumero, 10, '0');
	...
	Beneficiario.prototype.comDigitoNossoNumero = function (_digitoNossoNumero, banco, data) {
		// Sicred
		if(banco instanceof  Sicredi) {
			let Num = this.getAgenciaFormatada() + pad(this.getDigitoAgencia(), 2, '0');
    		if (this.getCodigoBeneficiario() != 0) {
      			Num = Num + pad(this.getCodigoBeneficiario(), 5, '0');
    		} else {
     			 Num = Num + pad(this.getConta(), 5, '0');
			}
        	Num = Num + pad(data.substring(6), 2, '0') +
        			2 + pad(this.getNossoNumero(), 5, '0');
			this._digitoNossoNumero = this.Modulo11(Num, 9, false);
		} // Sicoob
		else if(banco instanceof  Sicoob) {
			let indice = '319731973197319731973';
    		let Num =
 		   		this.getAgenciaFormatada() +
  		    	pad(this.getCodigoBeneficiario(), 9, '0') +
  		    	pad(this.getDigitoCodigoBeneficiario(), 1, '0') +
   		    	pad(this.getNossoNumero(), 7, '0');
    		let base = 0;
    		for (let i = 0; i < Num.length; i++) {
      			base = base + parseInt(Num.substring(i, i + 1)) * parseInt(indice.substring(i, i + 1));
    		}
    		let digito = 11 - (base - Math.floor(base / 11) * 11);
    		//(Se o Resto for igual a 0 ou 1 então o DV é igual a 0)
    		if (digito > 9) digito = 0;
    		this._digitoNossoNumero = digito.toString();
		} // Santander
		else if (banco instanceof Santander){
			let Num = pad(this.getNossoNumero(),12,'0')
			this._digitoNossoNumero = this.Modulo11(Num,9,false)
		} // Banco do Brasil
		else if (banco instanceof BancoBrasil){
			let Num = pad(this.getCarteira(),2,'0') + pad(this.getNossoNumero(),11,'0');
			let digito = this.Modulo11(Num,9,true);
			if (digito == 10 ) {
				this._digitoNossoNumero = 'X';
			}
			else {
				this._digitoNossoNumero = digito;
			}
		} else if (banco instanceof Itau) {
			let Num = this.getAgenciaFormatada() + pad(this.getConta(),5,'0') +
				pad(this.getCarteira(),3,'0') + pad(this.getNossoNumero(),8,'0');
			this._digitoNossoNumero = GeradorDeDigitoPadrao.mod10(Num);
		} else if (banco instanceof Caixa) {
			let Num;
			if (this.getCarteira() == 'SR') {
           		Num = '24' + pad(this.getNossoNumero(),15,'0')
			}
      		else if (this.getCarteira() == 'RG') {
           		Num = '14' + pad(this.getNossoNumero(),15,'0')
	  		}
      		else
           		Num = pad(this.getNossoNumero(),15,'0');
   			this._digitoNossoNumero = this.Modulo11(Num,9, false);
		}
		else this._digitoNossoNumero = _digitoNossoNumero;
		return this;
	};
	...
	Beneficiario.prototype.getConta = function () {
		return this._conta;
	};

	Beneficiario.prototype.comConta = function (_conta) {
		this._conta = _conta;
		return this;
	};

	Beneficiario.prototype.getDigitoConta = function () {
		return this._digitoConta;
	};

	Beneficiario.prototype.comDigitoConta = function (_digitoConta) {
		this._digitoConta = _digitoConta;
		return this;
	};

	Beneficiario.prototype.Modulo11 = function (Valor, Base, Resto) {
		let Soma = 0;
		let Peso = 2;
		for (let i = Valor.length; i > 0; i--) {
		  Soma = Soma + parseInt(Valor[i - 1]) * Peso;
		  if (Peso < Base) {
			Peso = Peso + 1;
		  } else {
			Peso = 2;
...
var Boleto = (function () {
	...
	Boleto.prototype.getAceiteFormatado = function () {
		if(this._banco instanceof Sicredi) {
			return this._aceite ? 'SIM' : 'NÃO';
		}
		else {
	...
	Boleto.prototype.comLocaisDePagamento = function (_locaisDePagamento) {
		if (this._banco instanceof Bradesco) {
			_locaisDePagamento = 'PAGAVEL PREFERENCIALMENTE NAS AGENCIAS BRADESCO';
		} else if (this._banco instanceof BancoBrasil) {
			_locaisDePagamento = 'PAGÁVEL EM QUALQUER BANCO';
		} else if (this._banco instanceof Caixa) {
			_locaisDePagamento = 'PREFERENCIALMENTE NAS CASAS LOTERICAS ATÉ O VALOR LIMITE';
		} else if (this._banco instanceof Itau) {
			_locaisDePagamento = 'ATÉ O VENCIMENTO, PREFERENCIALMENTE NO ITAÚ E APÓS O VENCIMENTO, SOMENTE NO ITAÚ';
		} else if (this._banco instanceof Sicredi) {
			_locaisDePagamento = 'PAGAVEL PREFERENCIALMENTE EM CANAIS ELETRONICOS DA SUA INSTITUICAO FINANCEIRA';
		} else {
			_locaisDePagamento = 'QUALQUER BANCO ATÉ O VENCIMENTO';
		}
