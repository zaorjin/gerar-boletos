...
var Beneficiario = (function () {
  ...
  Beneficiario.prototype.comNossoNumero = function (_nossoNumero, banco) {
    if (banco instanceof Sicoob) _nossoNumero = pad(_nossoNumero, 7, "0");
      ...
      _nossoNumero = pad(_nossoNumero, 12, "0");
    else if (banco instanceof Sicredi) _nossoNumero = pad(_nossoNumero, 5, "0");
    else if (
      !(banco instanceof BancoBrasil) ||
      this.getNumeroConvenio().length < 7
    )
      _nossoNumero = pad(_nossoNumero, 11, "0");
    else
      _nossoNumero =
        this.getNumeroConvenio().substring(0, 7) + pad(_nossoNumero, 10, "0");
  ...
  Beneficiario.prototype.comDigitoNossoNumero = function (
    _digitoNossoNumero,
    banco,
    data
  ) {
    ...
    if (banco instanceof Sicredi) {
      let Num =
        this.getAgenciaFormatada() + pad(this.getDigitoAgencia(), 2, "0");
      ...
      Num =
        Num +
        pad(data.substring(6), 2, "0") +
        2 +
        pad(this.getNossoNumero(), 5, "0");
      ...
      for (let i = 0; i < Num.length; i++) {
        base =
          base +
          parseInt(Num.substring(i, i + 1)) *
            parseInt(indice.substring(i, i + 1));
      }
      ...
      let Num =
        pad(this.getCarteira(), 2, "0") + pad(this.getNossoNumero(), 11, "0");
      ...
      let Num =
        this.getAgenciaFormatada() +
        pad(this.getConta(), 5, "0") +
        pad(this.getCarteira(), 3, "0") +
        pad(this.getNossoNumero(), 8, "0");
      ...
      } else Num = pad(this.getNossoNumero(), 15, "0");
      this._digitoNossoNumero = this.Modulo11(Num, 9, false);
...
var Datas = (function () {
  ...
  Datas.prototype.comDocumento = function (_documento, locate = "usa") {
    ...
    if (!validarData(_documento)) {
      throw new Error(
        "O ano do documento deve ser maior que 1997 e menor que 2999"
      );
    }
...
var Boleto = (function () {
  var DATA_BASE_ANTIGA = new Date(1997, 9, 7);
  var DATA_BASE_NOVA = new Date(2025, 1, 22);
  var DATA_MUDANCA = new Date(2025, 1, 21);
  function Boleto() {}
  ...
  Boleto.prototype.getFatorVencimento = function () {
    ...
    if (vencimento <= DATA_MUDANCA) {
      dataBase = DATA_BASE_ANTIGA;
      fator = Math.floor((vencimento - dataBase) / (1000 * 60 * 60 * 24));
    } else {
      dataBase = DATA_BASE_NOVA;
      fator =
        Math.floor((vencimento - dataBase) / (1000 * 60 * 60 * 24)) + 1000;
    }
